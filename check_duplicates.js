...
async function main() {
  const guests = await prisma.guest.findMany({ select: { name: true, group: true }});
    ...
    for (let j = i + 1; j < guests.length; j++) {
      ...
      if (key1 !== key2) {
        ...
        if (dist <= 2 && maxLength > 4) {
          similarNames.push({ name1: g1.name, group1: g1.group, name2: g2.name, group2: g2.group, distance: dist });
        }
      }
    }
  }
